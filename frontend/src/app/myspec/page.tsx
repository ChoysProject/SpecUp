...
export default function MySpecPage() {
  ...
  const [profile, setProfile] = useState(null); // { name, ... } or null
  const [career, setCareer] = useState([]); // [{...}, ...] or []
  ...
  // 버튼 클릭 핸들러 예시
  const handleAdd = (section) => alert(`${section} 추가하기`);
  const handleEdit = (section) => alert(`${section} 수정하기`);

  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 32, background: "#fff" }}>
      {/* 프로필 */}
      <div style={{ display: "flex", alignItems: "center", gap: 32, borderBottom: "1px solid #eee", paddingBottom: 24 }}>
        <div style={{ flex: 1 }}>
          {profile ? (
            <>
              <div style={{ fontSize: 28, fontWeight: 700 }}>{profile.name}</div>
              <div style={{ color: "#888", margin: "6px 0" }}>{profile.birth} ({profile.age}세)</div>
              <div style={{ color: "#666", fontSize: 15, marginBottom: 8 }}>{profile.email} | {profile.phone}</div>
              <div style={{ color: "#666", fontSize: 15 }}>{profile.address}</div>
            </>
          ) : (
            <div style={{ color: "#bbb", fontSize: 18 }}>프로필 정보를 입력해주세요.</div>
          )}
        </div>
        <div>
          ...
          <button
            style={{ marginTop: 12, background: "#3182f6", color: "#fff", border: "none", borderRadius: 8, padding: "6px 18px", cursor: "pointer" }}
            onClick={() => profile ? handleEdit("프로필") : handleAdd("프로필")}
          >
            {profile ? "수정하기" : "추가하기"}
          </button>
        </div>
      ...
      <section style={{ marginTop: 32 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button
            style={{ background: "#3182f6", color: "#fff", border: "none", borderRadius: 8, padding: "6px 18px", cursor: "pointer" }}
          ...
          career.map((c, i) => (
            <div key={i} style={{ marginBottom: 16 }}>
              ...
              <ul style={{ color: "#888", fontSize: 14, marginLeft: 16 }}>
                {c.detail.map((d, j) => <li key={j}>{d}</li>)}
              </ul>
            </div>
          ))
        ) : (
